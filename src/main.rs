...
fn main() {
    ...
    for _ in 0..args.nombre_de_lignes {
        let mut ligne: String = format!("insert into {} ({}) values (", args.table, args.colonnes);
...
fn generer_valeur(type_valeur: &str, colonne: &str) -> String {
    // La variable résultat
    let mut res: String = String::new();

    // Si la valeur est de type texte
    if [&String::from("TEXT"), &String::from("VARCHAR")].contains(&&type_valeur.to_uppercase()) {
        // On ajoute des guillemets autour des textes
        res.push_str(&String::from("'"));
        
        // Si la valeur est un nom
        if [&String::from("NOM"), &String::from("NAME"), &String::from("LASTNAME"), &String::from("LAST NAME"), &String::from("LAST-NAME"), &String::from("LAST_NAME")].contains(&&colonne.to_uppercase()) {
        ...
        } else if [&String::from("PRENOM"), &String::from("FIRSTNAME"), &String::from("FIRST-NAME"), &String::from("FIRST_NAME"), &String::from("FIRST NAME")].contains(&&colonne.to_uppercase()) {
        ...
        } else if [&String::from("MOT DE PASSE"), &String::from("MOTDEPASSE"), &String::from("MDP"), &String::from("PASSWORD"), &String::from("PASSWD"), &String::from("MOT_DE_PASSE"), &String::from("MOT-DE-PASSE")].contains(&&colonne.to_uppercase()){
            ...
            res.push_str(random_word::get(Lang::Fr));
        }

        res.push_str(&String::from("'"));

    } else if [&String::from("INTEGER"), &String::from("INT")].contains(&&type_valeur.to_uppercase()) {

        if "AGE" == colonne.to_uppercase() {
            // On génère un âge entre 7 et 77 ans
            res.push_str(&fastrand::u8(7..77).to_string());

        } else if [&String::from("YEAR"), &String::from("ANNEE"), &String::from("AN")].contains(&&colonne.to_uppercase()) {
            res.push_str(&fastrand::u16(1881..2222).to_string());
